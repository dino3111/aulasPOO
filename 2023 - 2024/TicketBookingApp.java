...
public class TicketBookingApp {
	...
	private static void exercise2(PrintStream out) {
        ...
        try {
            Scanner input = new Scanner(new FileReader("ticket_bookings.txt"));
            input.nextLine();
            ...
            while(input.hasNextLine()) {
                ...
                int id = Integer.parseInt(partes[0].replace("Booking=", ""));
                int bilhetes = Integer.parseInt(partes[1].replace(" ", ""));
            }
            input.close();
        } catch (IOException e) {
